...
fun JsonObject.toEventEntity(): EventEntity {
    ...
    return EventEntity(
        ...
        estimatedIntensity = properties?.get("mmi")?.jsonPrimitive?.contentOrNull?.toBigDecimal(),
        communityIntensity = properties?.get("cdi")?.jsonPrimitive?.contentOrNull?.toBigDecimal(),
        communityResponseCount = properties?.get("felt")?.jsonPrimitive?.intOrNull,
        magnitude = properties?.get("mag")?.jsonPrimitive?.contentOrNull?.toBigDecimal(),
